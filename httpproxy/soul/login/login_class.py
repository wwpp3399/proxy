...
from httpproxy.soul.redisdir.redis_a import redis_obj,redis_obj2
...
class Counter:
    ...
    def response(self, flow: mitmproxy.http.HTTPFlow):
        if self.get_DeviceId_android == flow.request.pretty_url or self.get_DeviceId_ios == flow.request.pretty_url:
            ...
            if redis_obj.exists(start_smDeviceId):
                ...
                set_res_dict = {"code": 1100, "detail": {"deviceId": redis_obj.get(start_smDeviceId)},
                                "requestId": requestId}
                logger.info(
                    f"--------------------替换手机DeviceId为数据库中已经绑定的SM-Id---------------{redis_obj.get(start_smDeviceId)}")
                ...
                redis_obj.set(start_smDeviceId, value="")
                redis_obj2.set(start_smDeviceId, value="")
        if self.login_url in flow.request.pretty_url:
            ...
            redis_obj_values = [redis_obj.get(i) for i in redis_obj.keys()]
            logger.info(redis_obj_values)
            if sMDeviceId in redis_obj_values:
                logger.info(f'--------------数据库中查到此-------{sMDeviceId}------说明第二次登录-----')
                deviceid=None
                for i in redis_obj.keys():
                    if sMDeviceId == redis_obj.get(i):
                        deviceid = i
                        break
                logger.info(deviceid)
                # now_device_data = redis_obj.hgetall(sMDeviceId)
                if redis_obj2.get(deviceid) == phone:
                    logger.info(f'-------------对比数据库中{redis_obj2.get("deviceid")}与手机输入{phone}相同，-------------登录成功')
                    ...
                    redis_obj.delete(deviceid)
                    redis_obj2.delete(deviceid)
                ...
                if query_phone_info:
                    ...
                    # 返回被修改的 response
                    redis_obj.set(sMDeviceId, value=query_phone_info.get("smDeviceId"))
                    redis_obj2.set(sMDeviceId, value=phone)
